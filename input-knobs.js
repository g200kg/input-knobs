...
";
	...
	for(let i=elem.length-1;i>=0;--i){
		...
		el.mousemove=(ev)=>{
			let min=el.min,max=el.max,step=el.step;
			if(min=="") min=0;
			if(max=="") max=100;
			if(step=="") step=1;
			let dx=(el.dragfrom.y-ev.clientY)*(max-min)/128;
			el.value=(((el.dragfrom.v+dx)/step)|0)*step;
		...
		el.redraw=()=>{
			if(el.valueold!=el.value){
				let min=el.min,max=el.max;
				if(min=="") min=0;
				if(max=="") max=100;
				let v=(el.value-min)/(max-min);
				if(el.sprites>1)
